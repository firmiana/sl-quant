...
def get_reward(new_state, time_step, action, xdata, signal, terminal_state, epoch=0):
    ...
    if not terminal_state:
        ...
        reward = (xdata[forwardsetep][0] - xdata[time_step][0]) * val
...
indata = load_data()
epochs = 20
gamma = 0.2  # a high gamma makes a long term reward more valuable
alpha = 1
epsilon = 1
learning_progress = []
...
for i in range(epochs):
    ...
    while not terminal:
        ...
        if (random.random() < epsilon) and i != epochs - 1:  # maybe choose random action if not the last epoch
            action = np.random.randint(0, 2)  # assumes 4 different actions
        ...
        # Get max_Q(S',a)
        newQ = model(torch.tensor(new_state.reshape(1, 2)).float())
        maxQ = torch.max(newQ)
        y = torch.zeros((2))
        # y[:] = qval[:]
        ...
        y[action] = update  # target output
        # y[:] = update
        print(f"action {action}, update {update}")
